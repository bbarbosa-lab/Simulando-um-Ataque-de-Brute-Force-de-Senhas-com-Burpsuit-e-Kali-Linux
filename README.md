...
Primeiramente não será usado o Medusa, também não será usado uma porta FTP, mas sim um http. Será usado o o Sistema Operacional Kali Linux, software Burpsuite, em conjunto com a plataforma de testes do Damn Vulnerable Web Application, sendo um ataque de brute force onde será possível descobrir as credenciais de acesso do administrador, login e senha.

Enunciado do trabalho: "Atenção: Este desafio é flexível! Você pode seguir os cenários propostos (FTP, DVWA, SMB) ou adaptar à sua realidade: experimentar outras ferramentas, criar novas wordlists, explorar módulos/serviços diferentes, ou apenas documentar em detalhes o que aprendeu, com estudos, reflexões e exemplos de código. O mais importante é demonstrar seu entendimento e compartilhar sua jornada de aprendizado!"
...
1# Configuração do Kali Linux com BurpSuite já instalado. 
  ...
  Para criar o ambiente de testes seguro virtualizado usando a imagem do sistema Kali Linux usaremos o Oracle Virtual Box:
...
4# Configurando os campos de entrada do brute force 1
  ...
  Deixe a opção de segurança no nível "low" outros níveis de segurança não funcionarão nessa configuração de brute force.
